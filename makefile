...
VERSION   ?= $(call get_gl_version,davical-project/davical,r)
AWLVRSN   ?= $(call get_gl_version,davical-project/awl,r,AWLVERSION)
...
TESTCMD   := \
	...
	php --version; \
	php-fpm -v; \
	echo "AWL     version: $$(cat /opt/davical/awlversion)"; \
	echo "DAViCal version: $$(cat /opt/davical/version)"; \
	#
...
BUILDFLAGS ?= \
	...
	--build-arg AWLVERSION=$(if $(AWLVRSN),$(AWLVRSN),$(error AWLVRSN is not defined)) \
...
OTHERFLAGS := \
	...
	# -e DAVI_ADMMAIL=calendar-admin@example.com \
	# -e DAVI_APPUSER=davical_app \
	# -e DAVI_APPPASS=insecurebydefault \
	# -e DAVI_HOSTNAME=localhost \
	# -e DAVI_LOCALE=en_US \
	# -e DAVI_SYSTNAME="DAViCal CalDAV Server" \
	# \
	# -e DAVI_DBAUSER=davical_dba \
	# -e DAVI_DBAPASS=insecurebydefault \
	# -e DAVI_SCHEMA=davical_dba \
	# \
	# -e WEBADMIN=admin \
	# -e PASSWORD=nimda \
	# \
	# -e PGHOST=localhost \
	# -e PGHOST=docker_postgresql --link docker_postgresql \
	# -e PGPORT=5432 \
	# -e POSTGRES_USER=postgres \
	# -e POSTGRES_PASSWORD=insecurebydefault \
	# \
	# -e DAVI_SKIP_DBSETUP=1 \
	# -e DAVI_SKIP_MIGRATION=1 \
	# \
...
get_config: ## get default config from github
	curl \
		...
		-SL https://gitlab.com/davical-project/davical/-/raw/master/config/example-config.php
...
# gets latest release version from gitlab
# $1 = gitlab repo in format: <user>/<repo>, e.g just-containers/s6-overlay
# $2 = filter releases by string ('^str' for start of string or 'str$' for end-of-string matching)
# $3 = filename relative to current dir, defaults to VERSION
# to bypass gitlab rate limit, cache value in $(CURDIR)/VERSION
# and dont refetch if file newer than 8 hours
define get_gl_version
$(shell \
	URL=$(if $(1),https://gitlab.com/api/v4/projects/$(subst /,%2f,$(1))/repository/tags?search=^$(2)\&order_by=updated\&sort=desc,$(error repo is not provided));
	OFILE=$(CURDIR)/$(if $(3),$(3),VERSION);
	if [ ! -f $${OFILE} ] \
